...
import {nonfungiblePositionManagerAddress, swapRouterAddress} from "../../constants";
...
import {fetchBlockTimestamp, fetchLatestPrice, fetchTokenPrices} from "../helpers";
import {NextResponse} from "next/server";

const textInput = <TextInput key={0} placeholder={'Search address, username or fid'}/>
const statsButton = <Button key={1} value={'stats'}>My Stats</Button>
const dexscreenerButton = <Button.Link key={2} href={"https://dexscreener.com/base/0xc9034c3e7f58003e6ae0c8438e7c8f4598d5acaa"}>Degen Chart</Button.Link>
// todo: embed the user's address in the share link
const shareButton = <Button.Link key={3} href={"https://warpcast.com/~/compose?text=Paper%handed%$degen?%Check%below!&embeds[]=https://degenpaperhands.xyz"}>Share</Button.Link>
const lookupButton = <Button key={4} value={'lookup'}>🔎</Button>
...
export const getValueHandler = async (c: FrameContext) => {
    ...
    if (buttonValue === 'lookup') {
        ...
        if (isAddress(inputText)) {
            ...
            // todo: this is the fid path
        } else {
            // todo: this is the farcaster name path
        }
    } else {
        const { verifiedAddresses, fid: fid_ } = c.var.interactor as {
            verifiedAddresses: { ethAddresses: Address[]; solAddresses: string[] }
            fid: number
        }
    ...
    const [toTransfers, fromTransfers, latestPrice] = await Promise.all([
        getDegenTransfersIn(user),
        getDegenTransfersOut(user),
        fetchLatestPrice()
    ]);

    console.log('toTransfers:', toTransfers)
    ...
    const airdropAddresses = [
        '0xc4794f15f05d903bcd4ffebe1baeeefbc5f801cc'
    ]

    const airdrops = toTransfers.filter(transfer => airdropAddresses.includes(transfer.from.toLowerCase()))
    console.log('airdrops:', airdrops)
    const fromTransfersExcludingNFT = fromTransfers.filter(transfer => transfer.to.toLowerCase() !== nonfungiblePositionManagerAddress.toLowerCase())
    ...
    if (!fromTransfersExcludingNFT) {
        ...
        return c.res({
            image: <img src={`https://degenpaperhands.xyz/api/diamondHands/${currentPortfolioValue}`} />,
            intents,
        })
    }
    ...
    allTransfers = allTransfers.filter((transfer) => transfer.to.toLowerCase() !== nonfungiblePositionManagerAddress.toLowerCase() && transfer.from.toLowerCase() !== nonfungiblePositionManagerAddress.toLowerCase());
    ...
    const lastPrice = pricesArray.pop()

    console.log('pricesArray:', pricesArray)
    console.log('latestPrice:', lastPrice)
    ...
    const allSaleTransfers = allTransfers
        ...
        .filter(transfer => transfer.to.toLowerCase() === swapRouterAddress.toLowerCase());
    ...
    const allBuyTransfers = allTransfers
        .map((transfer, index) => ({ ...transfer, index })) // Append the original index to each transfer
        .filter(transfer => transfer.from.toLowerCase() === swapRouterAddress.toLowerCase());
    ...
    const image = () => {
        if (+lostValue < 0) {
            return <img src={`https://degenpaperhands.xyz/api/paperHands/${lostValue}/increase`} />
        }
        if (+lostValue === 0) {
            return <img src={`https://degenpaperhands.xyz/api/diamondHands/${currentPortfolioValue}`} />
        }
        return <img src={`https://degenpaperhands.xyz/api/paperHands/${lostValue}`} />
    }
